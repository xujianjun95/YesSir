...
Chrome 扩展，Manifest V3，极简标签页管理。当前版本 **v1.5.5**（更新日志见 `CHANGELOG.md`）。
...
- **统一动画规范（雾化浮层系统）**：
  - **easing**：全局统一 `cubic-bezier(0.16, 1, 0.3, 1)`，优雅漂浮感
  - **分速原则**：transform 最快收尾，opacity 中间，filter/blur 最慢留余韵——模拟真实世界「位移先停、消散后到」
  - **主面板出现**：`opacity 0→1, translateY(10px→0), blur(8→0)`，320ms 级别（transform 260 / opacity 340 / filter 380）
  - **主面板消失**：`opacity 1→0, translateY(0→6px), blur(0→8px)`，260ms 级别（transform 220 / opacity 280 / filter 320）
  - **置顶槽**：比主面板晚 40ms，从侧边 `translateX(±8px)` 浮现，blur 极轻（6px）；消失与面板同步
  - **槽位卡片**：stagger 30ms + 20ms/个，只做 opacity + translateY，不加 blur
  - **拖拽吸附**：360ms，floatEl 飞入，槽位不动，是系统中唯一「有情绪」的动画
  - **pinned card 出现**：`transform 220ms, opacity 260ms`，像「接管」而非「出现」
  - **hover**：220ms，只做 background/shadow 微变，不做 translate/scale
  - **搜索栏**：不做独立动画，只保留 border/shadow 过渡跟随面板呼吸
...
- **三幕剧动画**（`cubic-bezier(0.16, 1, 0.3, 1)`）：
  - **Stage 1 感知**：拖拽中光标距面板边 < 80px → 该侧 `box-shadow` 渐显 `--ys-accent-glow`，给用户「这边可以放」的预判信号
  - **Stage 2 扩张**：跨过边界 → 主面板宽度 `700 → 740px`、横移 `±132.5px`，同步 SHOW_DURATION apple-ease（不是被推开，而是主动让位）
  - **Stage 3 浮现**：dock 从 `opacity(0) blur(6px) translateX(±8)` → `opacity(1) blur(0) 0`，比主面板晚 40ms；3 个 slot 额外 30ms + 20ms stagger（`translateY(6px) blur(4px)` → `0/0`）
- **隐藏动画**：dock 与主面板同步退场，transform 220ms / opacity 280ms / filter 320ms 分速收尾
- **关键常量**（`04-show-switcher.js` 顶部）：`CARD_WIDTH_BASE/EXPANDED`、`DOCK_WIDTH`、`DOCK_GAP`、`CARD_SHIFT`、`DOCK_SLIDE`、`EDGE_SENSE_DIST`、`SHOW_DURATION`、`HIDE_DURATION`、`APPLE_EASE`、`DOCK_DELAY`
...
- **拖动交互**：从 slot 拖出 → 垃圾桶动画 + 删除；slot 之间拖动 → floatEl 飞向目标（360ms）+ 槽位微光响应；点击 → 跳转对应标签；slot hover 仅 background/shadow 微变（无 scale/translate）
